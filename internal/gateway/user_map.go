...
type UserMap interface {
	...
	DeleteClients(userID string, clients []*Client) (deleted bool)
	UserState() <-chan UserState
	GetAllUserStatus(deadline time.Time, now time.Time) []UserState
}
...
func (u *userMap) DeleteClients(userID string, clients []*Client) (deleted bool) {
	if len(clients) == 0 {
		return false
	}
	u.lock.Lock()
	...
	for _, client := range tmp {
		if _, delCli := deleteAddr[client.conn.RemoteAddr().String()]; delCli {
...
func (u *userMap) GetAllUserStatus(deadline time.Time, now time.Time) (result []UserState) {
	u.lock.RLock()
	defer u.lock.RUnlock()
